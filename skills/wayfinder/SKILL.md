...
- **Prototype**: Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or UI/logic code via the /terebentina:prototype skill. Links the prototype as an asset. Use when "how should it look" or "how should it behave" is the key question.
- **Planing**: Conversation with the agent. Uses the /terebentina:lets-plan-code and /terebentina:domain-modeling skills. Asks one question at a time. The default case.
...
1. Run a `/terebentina:lets-plan-code` and `/terebentina:domain-modeling` session to surface the open decisions.
